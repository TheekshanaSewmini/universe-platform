...
export default function MyMaterialsList({ currentUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (file) {
      ...
      const materialType = (form.type || "OTHER").toUpperCase();
  ...
  const handleEdit = (material) => {
    setForm({
      ...
      subjectId: material.subjectId ?? material.subject?.id ?? selectedSubjectId,
    });
            ...
            {!editingId && (
              <div className="file-input">
                <label htmlFor="material-file">
                  📎 Upload file {form.type === "PDF" && "(PDF only)"}
                  {form.type === "VIDEO" && "(Video only)"}
                  {form.type === "NOTE" && "(PDF or image)"}
                  {form.type === "OTHER" && "(Any file)"}
                </label>
                <input
                  type="file"
                  id="material-file"
